...
const StoragePicker = () => {
  const {product, selectedStorage, setSelectedStorage} = useContext(DetailContext);
  ...
  const onKeyPress = (ev: React.KeyboardEvent<HTMLElement> | KeyboardEvent, option: StorageOption) => {
    if (ev.key === 'Enter') {
  ...
  return (
    <div className="flex flex-col font-light">
      <div className="text-14 font-light xl:mb-6">STORAGE. HOW MUCH SPACE DO YOU NEED?</div>
        ...
        {options &&
          options.map((option, index) => (
            <div
              ...
              tabIndex={0}
              style={{
                borderColor: selectedStorage.capacity === option.capacity ? 'black' : '#cccccc',
              }}
